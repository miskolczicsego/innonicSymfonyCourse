...
class PostControllerTest extends WebTestCase
{
    ...
    public function testCompleteScenario()
    {
        ...
        $authorValue = $crawler->filter('#blog_modelbundle_post_author option:contains("David")')
            ->attr('value');
        ...
        $this->assertGreaterThan(0, $crawler->filter('td:contains("New post")')->count(), 'The new post is not 
        showing up');

        // Edit the entity
        $crawler = $client->click($crawler->selectLink('Edit')->link());

        $form = $crawler->selectButton('Edit')->form(array('blog_modelbundle_post[title]' => 'Updated post',));

        $client->submit($form);
        $crawler = $client->followRedirect();

        // Check the element contains an attribute with value equals "Foo"
        $this->assertGreaterThan(0, $crawler->filter('[value="Updated post"]')->count(), 'The edited post is not 
        showing up');
